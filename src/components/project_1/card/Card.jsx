...
const Card = (props) => {
    ...
    useEffect(() => {
        async function add() {
            ...
            onValue(ref(db, `/guide/1/event/${props.id}/count/countsPlan`), (snapshot) => {
                setContent((prevContent) => ({
                    ...prevContent,
                    countsPlan: snapshot.val(),
                }));
            });

            onValue(ref(db, `/guide/1/event/${props.id}/count/countsGo`), (snapshot) => {
                setContent((prevContent) => ({
                    ...prevContent,
                    countsGo: snapshot.val(),
                }));
            });
//============
            const arrPrice = ["amount", "currency"]
            arrPrice.forEach((item) => {
                onValue(ref(db, `/guide/1/event/${props.id}/price/${item}`), (snapshot) => {
                    setContent((prevContent) => ({
                        ...prevContent,
                        [item]: snapshot.val(),
                    }));
...
//==============
            const arrData = ["number", "month", "weekDay"]
            arrData.forEach((item) => {
                onValue(ref(db, `/guide/1/event/${props.id}/data/${item}`), (snapshot) => {
                    setContent((prevContent) => ({
                        ...prevContent,
                        [item]: snapshot.val(),
                    }));
        ...
        <div className={styles.card__container}>
            ...
            <div className={styles.content}>
                ...
                <div className={styles.question__container}>
                    <h2 className={styles.question__container_title} id={"title"}>{`${content.title}`}</h2>
                    <p className={styles.question__container_description}
                       id={"smallDescription"}>{`${content.smallDescription}`}</p>
                    <div className={styles.level__container}>
                        ...
                        <p className={styles.level__container_description}>Уровень сложности материала: <span
                            id={"complexity"}>{`${content.complexity}`}</span></p>
                    </div>
                </div>
                ...
                <div className={styles.day__container}>
                    <div className={styles.info__container}>
                        {/*переделать дату!!!*/}
                        <i className={styles.info__container_number} id={"data/number"}>{`${content.number}`}</i>
                        <p className={styles.info__container_description}>
                            <span id={"data/month"}></span>{content.month}<span
                            id={"data/weekDay"}>{content.weekDay}</span>
                        </p>
                    </div>
                    ...
                    <div className={styles.guide__container}
                        ...
                        <div className={styles.guide__description_block}>
                            ...
                            <p className={styles.guide__description_block_desc} id={"name"}>{`${content.name}`}</p>
                        </div>
                    </div>
                </div>
                ...
                <ul className={styles.price__container}>
                    <li className={styles.price__container_item}>
                        ...
                        <p className={styles.price__container_description} id={"city"}>{content.city}</p>
                    </li>
                    <li className={styles.price__container_item}>
                    ...
                    <li className={styles.price__container_item}>
                        ...
                        {content.amount && content.currency ? (
                            <p className={styles.price__container_description}>
                                <span id={"price/amount"}>{content.amount} </span>
                                <span id={"price/currency"}>{content.currency}</span>
                            </p>
                        ) : (
